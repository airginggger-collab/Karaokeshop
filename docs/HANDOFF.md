...
## Предыдущая сессия (2026-06-23) — иконки с карточек убраны + поиск в хедере

Главная (`page.tsx`): удалены circle-иконки с карточек «Для дома», «Для бизнеса», «Калькулятор», «Песни», всех карточек секции «Разделы» (6 шт.) и секции «Почему karaokeshop» (4 шт.). Разделы рефакторены в `.map()`.

Хедер (`Header.tsx`): кнопка поиска теперь открывает `SearchOverlay` — новый компонент с inline-поиском по товарам и блогу. Пустой запрос → быстрые ссылки. Есть результат → список товаров + статей с переходом. Закрывается по Esc, клику вне панели. `SearchOverlay.tsx` создан в `src/components/`.

## Предыдущая сессия (2026-06-23) — ProductCard: убрана WhatsApp-кнопка

`ProductCard` — удалена зелёная кнопка «Узнать цену» (навязчиво, цена уже показана). Заменена на «Подробнее →» строкой рядом с ценой. `"use client"` убран (компонент стал серверным). WhatsApp CTA остаётся на странице товара.
