...
## Последняя сессия (2026-06-23) — авто-новости + cron деплой

`src/lib/news.ts` — build-time fetch Google News RSS по запросам «карaоке казахстан». Если RSS недоступен — показываются 6 fallback-новостей из site.ts. `NewsSection.tsx` — серверный компонент, встроен в конец `/blog`. CI cron: `0 8 * * *` (08:00 UTC = 11:00 Алматы) — ежедневная пересборка + деплой обновляет новости.

## Предыдущая сессия (2026-06-23) — дилерская полоса + обновлён бейдж героя

Герой: бейдж «Эксперты с 2012» → «Официальный дилер AST и Studio Evolution». Новая дилерская полоса под бенто-сеткой: бейджи AST и Studio Evolution + «С 2012 года · Алматы». Футер: добавлены бейджи дилера над копирайтом.

## Предыдущая сессия (2026-06-23) — Lottie-анимация в герое
