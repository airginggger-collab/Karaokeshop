...
- `LandingPage.tsx` (шаблон для `komplekty` индекс, `karaoke/[scenario]`, `(pages)/[page]`) — добавлена подсветка первого слова h1 (`splitH1`) один раз в шаблоне, а не в каждой странице.
