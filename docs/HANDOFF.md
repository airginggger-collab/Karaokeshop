...
>
> **Обновлено: 2026-07-06 — Task 6c дизайн-спринта «Сцена» (полировка a11y/perf + анти-AI правила в CLAUDE.md — ДИЗАЙН-СПРИНТ «СЦЕНА» ЗАВЕРШЁН, Task 1–6); до этого Task 6b (чистка мёртвого кода), Task 6a (визуальные must-фиксы), Task 5 (лендинги, 2026-07-05).** Ветка `main` — актуальная, деплоится на Cloudflare автоматически. Сборка чистая, тесты зелёные (web 24 + ui 2), CI/деплой зелёный.
>
...
>
> 🎨 **Текущий облик («Сцена», 2026-07-06 — Task 6c, спринт завершён):** тема по умолчанию — тёмная. Тёмная база: страница `#0b0913`, карточки `#171226`. Фиолетовая палитра от марки лого: `cta` `#7250f8`/`#6742e8`, `primary` `#9d8bf0`/`#4c30b8`, `hot` (скидки) `#ff5c8a`/`#d6296b` (токены `--color-*`, флипаются по теме через `.dark`). Фирменный приём — подсветка строки (`<HighlightLine>`/`.hl`) на ключевом слове заголовков и в nav. Тени-«клей» убраны — разделение поверхностей через hairline `border border-border`, тень только у плавающих элементов (FAB/оверлеи/sticky). Единый радиус `rounded-xl` (шкала до 12px, `rounded-2xl/3xl` запрещены). Тикеры (`.ticker`) вместо рядов pill-чипов для фактов доверия. Сток-фото удалён — карточки/фото без реального `image` показывают типографический плейсхолдер на `bg-scene`. Выдуманные отзывы/рейтинги товаров/логотипы клиентов скрыты под флагами `SHOW_UNVERIFIED_*` (сейчас `false`). Логотип-марка: фиолетовый квадрат `#7c5cff` + белый микрофон (Header + фавикон). WhatsApp-CTA — зелёный `#25D366` (фиксированный, не токенизирован намеренно). CountUp-компонент остаётся в репо (тесты), но в UI не используется — цифры статикой в тикерах.
>
...
## Последняя сессия (2026-07-06) — Task 6c дизайн-спринта «Сцена»: полировка a11y/perf + доки (ЗАВЕРШЕНИЕ СПРИНТА)
