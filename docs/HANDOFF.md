...
- **Светлая тема — тинт карточек:** мятный градиент `.bg-background` ([globals.css](../apps/web/src/app/globals.css)) заменён на мягкий синий `#d6e4f5` (из night-семейства) — по запросу; тёмная версия градиента (синяя) не тронута.
