...
>
> 🗺️ **Дорожная карта (решение владельца 2026-07-06):** порядок работ — **(A) Личный кабинет владельца** (следующая крупная задача; заказан «редактировать всё самой»; требует брейнсторма — сайт статический `output:export`, редактирование = вариант B: вынос контента из `site.ts` + CMS-слой/админка на Cloudflare; та же инфраструктура закроет «заявки не сохраняются» из аудита). Затем **вместе (B+C): второй проход анти-AI** (`stsena-backlog.md`: стат-гриды, повтор «иконка-в-кружке», симметрия hero) **+ закрытие аудит-дыр «для друзей»** (аналитика — Метрика-заглушка/GA4/GSC; юридика — политика/оферта/реквизиты РК). Аудит целиком — [«последняя сессия 2026-07-04»](#) выше в тексте и отчёты сессии.
>
...
## Последняя сессия (2026-07-06) — ЛК Фаза 1, Task 5: блог(26)/истории(8)/кейсы(3)/песни(8) → content/*.json
