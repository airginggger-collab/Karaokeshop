...
- **Task 2 (шелл) выполнен:** HighlightLine (SSR-safe подсветка строки), активные пункты Header/MobileNav через .hl, тикер в Footer вместо бренд-пилюль, Button без теней (primary brightness-ховеры, ghost прозрачный). ⚠️ Пост-ревью фикс: в MobileNav `.hl`'s `padding` (shorthand) перебивал `px-4 py-3.5` активного пункта и схлопывал тач-зону (56px→~1px) — добавлены `!px-4 !py-3.5` поверх `.hl` на активной ссылке, тач-зона подтверждена превью (56px, padding 14px/16px).
