...
>
> 🗺️ **Дорожная карта (решение владельца 2026-07-06):** порядок работ — **(A) Личный кабинет владельца** (заказан «редактировать всё самой»; сайт статический `output:export`, редактирование = вариант B: вынос контента из `site.ts` + CMS-слой/админка на Cloudflare; та же инфраструктура закроет «заявки не сохраняются» из аудита). **Фаза 1 (вынос контента в `apps/web/content/*.json`) ЗАВЕРШЕНА (Task 1–6, 2026-07-06)** — фундамент готов. **Фаза 2 (CMS-слой/визуальный `/admin`, Sveltia) — следующая крупная задача**, требует брейнсторма (см. [`docs/client/admin-guide.md`](client/admin-guide.md)). Затем **вместе (B+C): второй проход анти-AI** (`stsena-backlog.md`: стат-гриды, повтор «иконка-в-кружке», симметрия hero) **+ закрытие аудит-дыр «для друзей»** (аналитика — Метрика-заглушка/GA4/GSC; юридика — политика/оферта/реквизиты РК). Аудит целиком — [«последняя сессия 2026-07-04»](#) выше в тексте и отчёты сессии.
>
...
## Последняя сессия (2026-07-07) — ЛК Фаза 2, Task 1: content/*.json → {items:[…]} под Sveltia file-коллекции
