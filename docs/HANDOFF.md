...
- ⚠️ **Тот же паттерн (форс mood-night на всю страницу) ещё на:** `komplekty`, `komplekty/[area]`, `karaoke/[scenario]` (warm/night). НЕ трогал — это дизайн-решение; если нужно, чтобы и они следовали теме — отдельной задачей. `Mood` как АКЦЕНТ-карточка (бенто на главной) остаётся ок.
