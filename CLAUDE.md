...
- **Live (боевой прод):** https://www.karaokeshop.kz/ — = HEAD `main`, проверять его. ✅ Домен привязан 2026-07-21 (зона на Cloudflare, NS у ps.kz → `logan/veda.ns.cloudflare.com`, custom domains воркера + Redirect Rule apex→www + Always Use HTTPS). Технический хост https://karaokeshop.airg-inggger.workers.dev/ отдаёт тот же билд. Детали и откат — ловушка 7 и [docs/deploy.md](docs/deploy.md).
...
7. **Бренд-домен ПРИВЯЗАН (2026-07-21) — прод проверять на `https://www.karaokeshop.kz/`; канонизация хоста живёт в Cloudflare-дашборде, НЕ в репо.** История: до 2026-07-21 `.kz` отдавал старый Wix, canonical/og вели на чужой сайт. Сейчас: зона `karaokeshop.kz` в Cloudflare (`airg.inggger@gmail.com`, Free), NS у ps.kz — `logan/veda.ns.cloudflare.com`, оба хоста custom domains воркера (+`routes` в `wrangler.toml`), apex→www — **Redirect Rule** `apex-to-www-301` в дашборде (в `_redirects` нельзя, ловушка 0), http→https — **Always Use HTTPS** (SSL/TLS). Хвосты, которые видно только в дашборде и легко сломать по незнанию: удалишь Redirect Rule → apex станет 200-дублем www; выключишь Always Use HTTPS → http-дубли; managed-блок в начале `robots.txt` — норма (AI Crawl Control, наш `Sitemap:` в конце файла). Карта 301 со старого Wix — `public/_redirects` (секция 2). Откат: NS обратно на `ns1/ns2.wix.com` (пока жива подписка Wix). Чек-лист «Проверка после деплоя» — в [docs/deploy.md](docs/deploy.md).
 ...
 15. **Текст страницы, который называет модель/цифру, обязан сверяться с тем, что страница СЧИТАЕТ.** `/komplekty/do-80` годами обещал «комплект **AST-350**», а `bundleFor(80)` даёт **AST 250** (`pickBase(80)` → ast-250, предел ровно 80 м²): текст писали руками, состава на странице не было, и противоречить было нечему — вылезло сразу, как страницу наполнили. Защита: тест «если текст комплекта называет модель, она обязана быть в посчитанном составе» (`site.test.ts`, проверен на заведомо ошибочном тексте — падает). Там же пре-existing: **обе CTA на `/komplekty/*` были мёртвыми** (`<Button>` без `href`/`onClick`) — `<Button>` из `@kk/ui` это просто кнопка, для перехода **оборачивай в `<Link>`**, иначе клик молча ничего не делает; на страницах-сиротах такое живёт годами (аудит их не открывал). Данные комплектов (`area`, `scenario`) — в `content/bundles.json`, цена и состав **только** через `bundleFor()`.

 16. **`position: fixed` внутри `<header>` — не fixed: `.header-scene` держит `backdrop-filter`, а filter/backdrop-filter/transform/perspective/contain на ЛЮБОМ предке делает его containing block для fixed-потомков.** Бургер-drawer (`MobileNav`) лежал внутри шапки и на телефоне схлопывался до её коробки — **375×56 вместо 375×812**: меню рисовалось поверх страницы обрезком, футер drawer с WhatsApp-кнопкой вылезал на контент, фон не покрывал экран (найдено 2026-09-03 по скриншоту владельца). Ломается молча: сборка, типы и тесты зелёные, на десктопе не видно вообще. Защита: drawer уходит `createPortal(drawer, document.body)`, сторож в `components/mobile-layout.test.ts` (проверен на заведомо сломанном исходнике — падает). **Правило: любой полноэкранный оверлей/drawer рендерить порталом в `body`, а не там, где стоит триггер.** Проверка не грепом, а `getBoundingClientRect()` в браузере: у настоящего fixed `offsetParent === null`, у пойманного — элемент-предок.
 ...
 20. **Линта в репо фактически НЕ БЫЛО: `npm run lint` уходил в интерактивный визард и падал.** `next lint` без конфига ESLint спрашивает «How would you like to configure ESLint?», в неинтерактивной сессии это выход 1 — то есть команда из README не работала, а в CI её нет, поэтому никто не замечал. Настроено 2026-09-04: ESLint 9 flat config (`apps/web/eslint.config.mjs`, `packages/ui/eslint.config.mjs`), скрипты `lint` = `eslint .`, правила Next подключены через `FlatCompat` (`eslint-config-next` пока в старом формате; `next lint` deprecated и удаляется в Next 16). Первый же прогон нашёл 7 ошибок, из них две реальные: `as any` в `NewsSection` (починено) и `<a href="/brand/…">` в полосе брендов главной, то есть полная перезагрузка вместо клиентского перехода (фикс есть в рабочем дереве, но `page.tsx` в тот момент переписывала параллельная сессия — см. HANDOFF). Осознанные исключения — в конфиге с причиной: `no-img-element` выключен (статический экспорт, next/image не оптимизирует), `no-html-link-for-pages` выключен для `/admin-guide` (`/admin/` это статика Decap CMS в `public/`, а не роут Next, и `<Link>` увёл бы в 404). **Главная ловушка внутри ловушки: `next build` линтует САМ, как только в проекте появляется конфиг ESLint.** Шага линта в CI нет, но деплой всё равно упал сразу после этого коммита: `next build` → «Failed to compile» на `no-html-link-for-pages` → **прод застрял на прошлой версии** (тот же сценарий, что в ловушке 0). Поэтому в `next.config.mjs` стоит `eslint: { ignoreDuringBuilds: true }` — **не удалять**: линт живёт отдельной командой `npm run lint`, сборка и деплой от него не зависят. Гонять локально перед коммитом.
