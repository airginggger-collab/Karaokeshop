...
npm test -w web              # тесты web (Vitest): 72 (calculator + seo + quiz + site + wa + CountUp + mobile-layout + headers)
...
7. **Бренд-домен ПРИВЯЗАН (2026-07-21) — прод проверять на `https://www.karaokeshop.kz/`; канонизация хоста живёт в Cloudflare-дашборде, НЕ в репо.** История: до 2026-07-21 `.kz` отдавал старый Wix, canonical/og вели на чужой сайт. Сейчас: зона `karaokeshop.kz` в Cloudflare (`airg.inggger@gmail.com`, Free), NS у ps.kz — `logan/veda.ns.cloudflare.com`, оба хоста custom domains воркера (+`routes` в `wrangler.toml`), apex→www — **Redirect Rule** `apex-to-www-301` в дашборде (в `_redirects` нельзя, ловушка 0), http→https — **Always Use HTTPS** (SSL/TLS). Хвосты, которые видно только в дашборде и легко сломать по незнанию: удалишь Redirect Rule → apex станет 200-дублем www; выключишь Always Use HTTPS → http-дубли; managed-блок в начале `robots.txt` — норма (AI Crawl Control, наш `Sitemap:` в конце файла). Карта 301 со старого Wix — `public/_redirects` (секция 2). Откат: NS обратно на `ns1/ns2.wix.com` (пока жива подписка Wix). Чек-лист «Проверка после деплоя» — в [docs/deploy.md](docs/deploy.md).
 ...
 17. **Официальное фото товара приходит с ЗАПЕЧЁННЫМ фоном, и подложка `bg-scene` его не спасает — в тёмной теме выходит белая (или чёрная) плашка, то есть прямое нарушение UI-правила 6.** `ProductImage` красит контейнер в `bg-scene`, но если у самого JPEG/WebP фон залит белым, виден именно он. Жило незаметно, пока фото сидели в мелких карточках каталога; вылезло сразу, как снимок попал в hero главной (2026-09-04). Отдельная ловушка внутри ловушки: у `evobox-plus-main` фон **чёрный**, а не белый, поэтому «проверил один файл — почини все» не работает, полярность у каждого снимка своя. Защита: перед публикацией фото прогонять через вырезание фона заливкой **от рамки кадра** (не по порогу яркости глобально — иначе выест белый логотип EVOBOX внутри объекта и белое на стикере), альфу задавать как `255 − яркость` для белого фона и как `яркость` для чёрного, чтобы студийная тень растворялась вместо серого пятна. Проверять результат **на обеих подложках сразу** (`#1a2233` и `#eef1f6`), а не только в текущей теме. Скрипт разовый, живёт в скретчпаде сессии; в репо кладётся уже готовый WebP с альфой.


 21. **«Вёрстка уехала на телефоне» это ДВА разных дефекта, и лечатся они по-разному: у одного виноват layout, у другого визуальный зум. Пока не измерил, чинить нечего.** Жалоба 2026-09-10 («страница сдвинута вправо, слева срезан бургер, капс-строка обрезана с двух сторон») выглядела как классический горизонтальный overflow, но им не была: на проде и на локальном билде `document.documentElement.scrollWidth - window.innerWidth` = 0 на 320/390/440, на каталоге, калькуляторе, товаре и сравнении, в том числе с отключёнными веб-шрифтами и после открытия-закрытия бургера. Признак, по которому картинка отличается от overflow: контент срезан **с обеих сторон** и выглядит крупнее обычного, то есть уже не layout-вьюпорт, а **визуальный** (pinch-zoom на трекпаде внутри device mode, двойной тап на телефоне). При зуме layout не меняется, страница просто ездит вбок, и никакой CSS этого не «чинит». **Протокол:** сначала снять три числа на той самой странице и ширине, потом делать выводы:
     ```js
     ({ overflow: document.documentElement.scrollWidth - window.innerWidth,  // > 0 → это layout
        scale: visualViewport.scale,                                          // > 1 → это зум
        left: visualViewport.offsetLeft })                                    // > 0 → страница просто сдвинута
     ```
     `overflow > 0` — искать элемент: пройти `body *`, отбросить `position: fixed` (закрытый drawer лежит за левым краем и в список попадает всегда) и найти те, у кого `getBoundingClientRect().right > innerWidth`; выходы внутри `overflow-x`-контейнеров (табы каталога, таблица `/sravnenie`) это by design. `scale > 1` — дефекта в вёрстке нет, масштаб сбрасывается двойным тапом или Cmd+0. Отдельно: скриншот, снятый **в момент деплоя**, может показывать прошлый билд из кеша Cloudflare, поэтому первым делом жёсткая перезагрузка. **Что при разборе действительно нашлось:** `priceFmt` клеил валюту обычным пробелом (`"798 000" + " ₸"`), хотя разряды внутри числа Intl уже разделяет неразрывным, поэтому на узкой строке «₸» отрывался и уезжал на следующую строку. Теперь пробел перед валютой неразрывный (U+00A0), сторож в `site.test.ts` проверен на регрессе. Мораль шире: **любая склейка числа с единицей («₸», «м²», «шт.», «лет») делается неразрывным пробелом**, иначе перенос рвёт её на самом узком экране, а поймать это грепом невозможно.

 22. **Общая CSP в `public/_headers` молча убила кабинет `/admin/`: владелец не мог войти почти два месяца (2026-07-14 → 2026-09-10).** Security-ревью добавило на `/*` строгую CSP (`script-src 'self' … mc.yandex.ru`), а Sveltia CMS грузит бандл с `unpkg.com` и ходит в `api.github.com` → скрипт заблокирован, `/admin/` отдаёт 200 и **белую страницу** без формы входа. Сборка, тесты, Lighthouse и деплой зелёные, `curl` видит нормальный HTML: ошибка есть только в консоли браузера, а кабинет после ревью никто не открывал. Вторая половина ловушки: **CSP-правила в `_headers` не заменяют друг друга, а склеиваются через запятую** (две политики сразу, строгая побеждает), поэтому исключение для пути делается только так: `! Content-Security-Policy` (снять общую), затем своя строка. Сейчас у `/admin/*` отдельная CSP по списку хостов из [доков Sveltia](https://sveltiacms.app/en/docs/security) (unpkg, cdn.jsdelivr.net для шрифтов, api.github.com, *.githubusercontent.com), общая CSP сайта не ослаблена. Сторож: `src/lib/headers.test.ts` (хост скрипта из `admin/index.html` обязан быть в CSP кабинета; проверен на удалённом правиле, падает). **Правило: ужесточил CSP или заголовки, открой в браузере каждую страницу со сторонним скриптом (`/admin/`, `/kontakty` с картой) и посмотри консоль; `_headers` можно проверить локально через `npx wrangler dev`, он применяет правила как прод.** Та же CSP заодно резала бикон **Cloudflare Web Analytics** (`static.cloudflareinsights.com`, его вставляет сам Cloudflare на краю, в репо тега нет), и статистика не писалась с того же 14 июля; по решению владельца хост добавлен в `script-src` сайта (хост, не путь: реальный URL `beacon.min.js/v31…`), данные идут на свой домен `/cdn-cgi/rum` через `connect-src 'self'`. В CSP кабинета бикона нет намеренно, ошибка про него в консоли `/admin/` это норма. Сторож в том же `headers.test.ts`.
...
### Mobile-first (правило владельца 2026-09-10)

**Сайт проектируется и верстается от телефона.** Основная аудитория приходит с мобильного, поэтому мобильный вид это не «сжатый десктоп», а исходный макет; десктоп получается добавлением, а не наоборот.

- **Базовые классы Tailwind без префикса = телефон.** `sm:`/`md:`/`lg:` только надстройки поверх. Писать «десктопное значение по умолчанию, а на мобиле переопределю» запрещено: такие правила молча ломаются на промежуточных ширинах.
- **Первый экран телефона несёт смысл, а не украшения.** Заголовок, лид, цена, одна кнопка. Тяжёлое прячем до планшета: hero-фото стоит `hidden md:block lg:hidden` плюс правая колонка от `lg`, потому что на телефоне кадр отжимал заголовок и кнопку за сгиб (решение владельца 2026-09-10).
- **Тач-таргеты минимум 44px** (`min-h-11`/`h-11 w-11`, ловушка 19), **поля ввода минимум 16px** (`text-base md:text-sm`, иначе iOS Safari зумит страницу, ловушка 18).
- **Парные пороги менять только вместе:** `nav` в `Header.tsx` (`lg:flex`) и обёртка `MobileNav.tsx` (`lg:hidden`), иначе на промежуточной ширине выйдет два меню или ни одного (ловушка 13).
- **Полноэкранные оверлеи и drawer рендерить порталом в `body`** (ловушка 16): `backdrop-filter` на предке ломает `position: fixed`.
- **Проверять на 320 / 360 / 390 / 414 / 440**, не только на «мобильном» пресете. Горизонтальный overflow мерить в браузере, вёрстку смотреть скриншотом: грепом по HTML это не проверяется (ловушка 13).

```js
// горизонтальный скролл документа: должно быть 0 на каждой ширине
...
8. **Один WhatsApp-CTA на вьюпорт.** Иерархия: зелёный WhatsApp → акцентный CTA (`bg-cta`, зелёный `#41A876` в тёмной теме / чернильно-синий в светлой) → ghost.
...
### Пояснение секции — тултип, а не подзаголовок

Решение владельца 2026-09-10: строки-подзаголовки под заголовками секций убраны, текст живёт во всплывающей подсказке. Компонент — `SectionHint` (`src/components/SectionHint.tsx`), у `SectionTitle` на главной есть проп `hint`.

- Компонент **без состояния и без `"use client"`**: показ на `group-hover` + `group-focus-within`, поэтому работает в статическом экспорте без гидратации. Кнопка нужна для клавиатуры и тапа (на телефоне hover'а нет), текст для скринридера несёт `aria-label`, сам тултип `aria-hidden` (иначе читается дважды).
- **Позиционный контекст — обёртка заголовка** (`relative` на строке «заголовок + подсказка»), НЕ сам `SectionHint`. Тултип 288px тогда начинается от левого края заголовка и влезает даже на 320px; привязка к иконке уводит его за правый край, потому что иконка стоит в конце строки.
- Переходы перечислять явно (`transition-[opacity,transform]`), не `transition-all` — ловушка 17.
