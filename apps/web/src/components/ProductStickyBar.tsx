...
export function ProductStickyBar({ model, price }: ProductStickyBarProps) {
  const waText = encodeURIComponent(`Здравствуйте! Интересует ${model}, подскажите наличие и цену.`);
    ...
    <div className="fixed bottom-0 left-0 right-0 z-40 border-t border-border bg-background/95 px-4 py-3 backdrop-blur lg:hidden">
      <div className="flex items-center gap-3">
        <div className="flex-1">
          ...
          <p className="text-lg font-bold">{priceFmt(price)}</p>
        </div>
        <a
          href={waHref}
          target="_blank"
          rel="noopener noreferrer"
          className="flex items-center gap-2 rounded-xl bg-[#25D366] px-5 py-3 text-sm font-medium text-white"
        >
