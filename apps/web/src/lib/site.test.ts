...
// SEO-гигиена контента. Найдено аудитом 2026-07-21 на живом проде: (1) опечатка
// «карао» вместо «караоке» жила в 86 местах blog/stories (включая title статей),
// (2) 42 из 69 страниц имели title > 60 симв. и резались в выдаче Google.
// Бюджет: layout добавляет суффикс « | karaokeshop» (+14) → на страницу ≤ 48.
describe("SEO-гигиена контента (аудит 2026-07-21)", () => {
  const contentDir = path.join(__dirname, "..", "..", "content");

  it("опечатка «карао» (без «к») не возвращается ни в один content/*.json", () => {
    for (const f of fs.readdirSync(contentDir).filter((x) => x.endsWith(".json"))) {
      const raw = fs.readFileSync(path.join(contentDir, f), "utf-8");
      const bad = raw.match(/[Кк]арао(?!к)[^"]{0,30}/g);
      expect(bad, `${f}: ${bad?.join(" | ")}`).toBeNull();
    }
  });

  it("title статей блога и лендингов ≤ 48 симв. (суффикс layout добавит ещё 14)", () => {
    const metas: { where: string; title: string }[] = [
      ...blogPosts.map((p) => ({ where: `blog/${p.slug}`, title: p.title })),
      ...[oNasMeta, kontaktyMeta, sravnenieMeta, catalogMeta, podKlyuchMeta,
        komplektyIndexMeta, kalkulyatorMeta, pesniMeta, keysyMeta, blogMeta,
        dlyaDomaMetaV2, dlyaBiznesaMeta, gotovyeResheniyaMeta,
      ].map((m) => ({ where: m.slug, title: m.title })),
    ];
